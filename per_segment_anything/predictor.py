...
class SamPredictor:
    ...
    def set_image(
        ...
        if mask is not None:
            input_mask = self.transform.apply_mask(mask)
            input_mask_torch = torch.as_tensor(input_mask, device=self.device)
            input_mask_torch = input_mask_torch.reshape(input_mask_torch.shape[0], -1, 1)
            input_mask_torch = input_mask_torch.permute(2, 0, 1).contiguous()[None, :, :, :]

        input_mask, input_image = self.set_torch_image(
            input_image_torch,
            image.shape[:2],
            cal_image=cal_image,
    ...
    def set_torch_image(
        ...
        if transformed_mask is not None:
            input_mask = self.model.preprocess_mask(transformed_mask)  # pad to 1024
        return input_mask, input_image
    ...
    def predict(
        ...
        masks = [mask.detach().cpu().numpy() for mask in masks.squeeze()]
