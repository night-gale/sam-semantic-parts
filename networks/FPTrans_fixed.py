...
class FPTransFixed(nn.Module):
    def __init__(self, opt, logger):
        super(FPTransFixed, self).__init__()
        self.opt = opt
        self.logger = logger
        self.shot = opt.shot
        ...
        self.__class__.__name__ = f"FPTransFixed/{opt.backbone}"
        ...
        for var in self.encoder.parameters():
            var.requires_grad = False
        self.encoder[0].prompt_tokens.requires_grad = True
